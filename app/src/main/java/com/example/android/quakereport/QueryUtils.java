...
public final class QueryUtils {
    ...
    public static ArrayList<ArrayList> extractEarthquakes(String urls) {

        // Create an empty ArrayList that we can start adding earthquakes to
        ArrayList<ArrayList> EarthQuak = new ArrayList<>();
        ArrayList<String> earthquakes = new ArrayList<>();
        ArrayList<String> earthquakesdate = new ArrayList<>();
        ArrayList<String> earthquakesrang = new ArrayList<>();
        ArrayList<String> earthquakesUrl = new ArrayList<>();

        try {
            // TODO: Parse the response given by the SAMPLE_JSON_RESPONSE string and
            // build up a list of Earthquake objects with the corresponding data.

            JSONObject baseJsonResponse = new JSONObject(urls);
            JSONArray arrayrespond = baseJsonResponse.getJSONArray("features");
            for(int i=0; i<arrayrespond.length(); i++){
                JSONObject baseprop = arrayrespond.getJSONObject(i);
                JSONObject propenter = baseprop.getJSONObject("properties");
                String mag = propenter.getString("mag");
                String place = propenter.getString("place");
                String time = propenter.getString("time");
                String Url = propenter.getString("url");

                earthquakes.add(place);
                earthquakesdate.add(time);
                earthquakesrang.add(mag);
                earthquakesUrl.add(Url);
            }

            EarthQuak.add(earthquakesrang);
            EarthQuak.add(earthquakes);
            EarthQuak.add(earthquakesdate);
            EarthQuak.add(earthquakesUrl);
    ...
    //HttpConnect
    public static ArrayList<ArrayList> fetchEarthquakeData() {
        // Create URL object
